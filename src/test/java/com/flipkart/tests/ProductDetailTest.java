...
public class ProductDetailTest extends BaseTest {
    ...
    public void setUpPage() throws InterruptedException {
        ...
        List<WebElement> products = getDriver().findElements(
    ...
    public void verifyProductPageLoaded() {
        Assert.assertTrue(
            detailPage.isProductPageLoaded(),
    ...
    public void verifyProductTitleDisplayed() {
        Assert.assertTrue(
            detailPage.isProductTitleDisplayed(),
    ...
    public void verifyProductPriceDisplayed() {
        Assert.assertTrue(
            detailPage.isProductPriceDisplayed(),
    ...
    public void verifyProductImagesDisplayed() {
        Assert.assertTrue(
            detailPage.areProductImagesDisplayed(),
    ...
    public void verifyAddToCartButtonPresent() {
        Assert.assertTrue(
            detailPage.isAddToCartButtonPresent(),
    ...
    public void verifyBuyNowButtonPresent() {
        Assert.assertTrue(
            detailPage.isBuyNowButtonPresent(),
    ...
    public void verifySellerInfoDisplayed() {
        Assert.assertTrue(
            detailPage.isSellerInfoDisplayed(),
            "Seller info should be displayed!"
        );
    }
    ...
    public void verifyDeliveryInfoDisplayed() {
        Assert.assertTrue(
            detailPage.isDeliveryInfoDisplayed(),
            "Delivery info should be displayed!"
        );
    }
    ...
    public void verifyOffersDisplayed() {
        Assert.assertTrue(
            detailPage.areOffersDisplayed(),
            "Offers should be displayed!"
        );
    }
    ...
    public void verifySpecificationsDisplayed() {
        Assert.assertTrue(
            detailPage.isSpecificationsDisplayed(),
            "Specifications should be displayed!"
        );
    }
    ...
    public void verifyThumbnailClick() {
        int imagesBefore = getDriver()
            .findElements(By.cssSelector("img[src*='rukminim']"))
            .size();
        detailPage.clickThumbnailImage();
        int imagesAfter = getDriver()
            .findElements(By.cssSelector("img[src*='rukminim']"))
            .size();
        System.out.println("Images before: " + imagesBefore
            + " after: " + imagesAfter);
        Assert.assertTrue(imagesAfter > 0,
            "Images should still be visible after thumbnail click!");
    }

    // ─── TC51: Verify MRP displayed ──────────────────────────────
    @Test(description = "Verify MRP and discount are shown")
    public void verifyMrpDisplayed() {
        List<WebElement> mrp = getDriver().findElements(
            By.xpath("//*[contains(text(),'MRP')]"));
        System.out.println("MRP elements: " + mrp.size());
        Assert.assertTrue(
            mrp.size() > 0 ||
            detailPage.isProductPriceDisplayed(),
            "Price or MRP should be displayed!"
        );
    }
    ...
    public void verifySimilarProductsDisplayed() {
        ...
        List<WebElement> similar = getDriver().findElements(
            By.cssSelector("a[href*='/p/']"));
        System.out.println("Similar products: " + similar.size());
        Assert.assertTrue(similar.size() > 0,
