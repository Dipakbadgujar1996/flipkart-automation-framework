...
public class HomePage {
	...
	public void closeLoginPopupIfPresent() {
		try {
			...
			try {
				WebElement closeBtn = driver.findElement(By.cssSelector("span[role='button'].b3wTlE"));
			...
			try {
				WebElement closeBtn = driver.findElement(By.xpath("//span[@role='button' and text()='✕']"));
				((JavascriptExecutor) driver).executeScript("arguments[0].click();", closeBtn);
	...
	public boolean isLogoVisible() {
		try {
			List<WebElement> logos = driver.findElements(By.cssSelector("img[src*='rukminim']"));
	...
	public boolean areDealsVisible() {
		try {
			...
			Thread.sleep(2000);
			List<WebElement> cards = driver.findElements(By.cssSelector("a[href*='/p/']"));
			if (cards.size() == 0) {
				cards = driver.findElements(By.cssSelector("img[src*='rukminim']"));
			}
	...
	public void clickLogo() {
		try {
			wait.until(ExpectedConditions.elementToBeClickable(flipkartLogo));
			flipkartLogo.click();
		} catch (Exception e) {
	...
	public SearchResultsPage searchProduct(String keyword) {
		try {
			...
			// to avoid renderer timeout
			try {
				WebElement btn = driver.findElement(By.cssSelector("button[type='submit']"));
				((JavascriptExecutor) driver).executeScript("arguments[0].click();", btn);
				System.out.println("Search via JS click!");
			} catch (Exception e1) {
				// Fallback — press Enter key
				box.sendKeys(Keys.ENTER);
				System.out.println("Search via Enter key!");
			}
			...
			try {
				...
				System.out.println("Search fallback failed");
			}
		}
	...
	public SearchResultsPage searchProductUsingEnter(String keyword) {
		try {
			...
			box.sendKeys(Keys.ENTER);
			Thread.sleep(3000);
			System.out.println("Search via Enter!");
		} catch (Exception e) {
			System.out.println("Enter search failed: " + e.getMessage());
		}
		return new SearchResultsPage(driver);
	}
